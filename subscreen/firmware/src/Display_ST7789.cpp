...
void LCD_addWindow(uint16_t Xstart, uint16_t Ystart, uint16_t Xend, uint16_t Yend, uint16_t *color) {
  ...
  LCD_WriteData_nbyte((uint8_t *)color, numBytes);
}

static uint16_t backlight_duty(uint8_t percent) {
  if (percent > 100) {
    percent = 100;
  }
  const uint32_t max_duty = (1u << Resolution) - 1u;
  return static_cast<uint16_t>((max_duty * percent) / 100u);
}
...
void Set_Backlight(uint8_t Light) {
  ledcWrite(EXAMPLE_PIN_NUM_BK_LIGHT, backlight_duty(Light));
}
